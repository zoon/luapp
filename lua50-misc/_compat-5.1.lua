...
------------------------------------------------------------------------------
assert(string.find(_VERSION,"Lua 5.0"))

------------------------------------------------------------------------------
-- NOTE: This file is heavily modified. 
-- NOTE: Consider using fresh one from "www.keplerproject.org/compat"
------------------------------------------------------------------------------
_G.require50 = _G.require 
...
-- (lua-import '(assert error getfenv ipairs loadfile rawset rawget))
local assert = assert
local error = error
...
local rawset = rawset
local rawget = rawget
-- (lua-import '(loadlib pairs setfenv setmetatable type))
local loadlib = loadlib
local pairs = pairs
local setfenv = setfenv
local setmetatable = setmetatable
local type = type
--(lua-import'(string.find string.format string.gfind string.gsub string.sub))
...
package.path  = LUA_PATH or "?;./?.lua;./scripts/?.lua;./scripts/lib/?.lua"
...
local function findfile (name, pname)
  ...
  for c in gfind (path, "[^;]+") do
    ...
    --local f = io.open (c)
    local f, err = loadfile(c) -- TODO: change to fopen when bindings are ready
    if f then
      -- f:close ()
      return c
    end
...
function _PACKAGE.seeall (module)
  local t = type(module)
  assert (t == "table",
          "bad argument #1 to package.seeall (table expected, got "..t..")")
